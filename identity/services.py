"""Person creation, lifecycle acts, and the project's first rate limit (issue #327, #482, #483, #487, ADR 0018).

`create_person_with_temp_password` (issue #482) is the only creation
...
instead of account creation. `add_person` stages a Person with no usable
password and no credential at all, for an admin building a roster ahead
of actually onboarding someone.

Issue #487 (ADR 0018) retired the email-based alternative these three
replace: `invite_person()`'s emailed one-time set-password link,
`resend_invite()` (recovering a dead invite), and the outbound-auth-email
rate limit (`is_auth_email_rate_limited()`/`AuthEmailRequest`) that
guarded it. Nothing in this project sends a credential by email any more;
`git log` is the record of that path, the same way issue #341's cutover
left the pre-SPA template tree only in history.

`is_login_rate_limited()` is unrelated to any of that — it throttles
failed sign-ins, keyed on submitted address and requesting IP, and stands
unchanged.
"""
...
def create_person_with_temp_password(*, name, email):
    ...
    loggable-in the instant this returns — there is no separate "accept
    the invite" step left to wait on. `must_change_password=True` is the
    durable record that the password was admin-generated rather than
    member-chosen; it's cleared the moment the member successfully sets
    their own (issue #333's change-password flow, issue #487's forced-
    change gate).

    Sends no mail and reaches no external service at all, so there's
    nothing here that needs `transaction.on_commit()` protection during an
    ADR-0008 Preview: a rolled-back transaction discards the Person row
...
def add_person(*, name, email):
    """Create a Person with no usable password and no credential issued at all (issue #397).

    Lets an admin stage a semester roster — add people, assign their Roles
    and castings — ahead of actually onboarding them with a real
    credential via `create_person_with_temp_password()` or
    `apply_password_reset()`. Wrapped in `transaction.atomic()` even
...
def apply_password_reset(*, target, requesting_admin):
    ...
    any time, having collapsed the old email flow's "recover a dead
    invite" job into itself. Sets a real, immediately-usable password via
    `set_password()` (not an unusable one) so the target can sign in the
    instant the admin relays it, and flags `must_change_password=True` —
    the same durable "was admin-generated" record
    `create_person_with_temp_password()` sets, cleared the moment the
    member successfully changes their own password.
...
def invite_status_for(person):
    ...
    A Person is loggable-in from the moment they're created (a real temp
    password, never an unusable one), so the only lifecycle fact worth
    surfacing is whether that password is still the admin-generated one
    they haven't replaced yet.
    """
    return 'must_change_password' if person.must_change_password else 'active'


def clear_must_change_password(person):
    """Set `person.must_change_password = False` (issue #484): the one place that clears the temp-password gate.

    Mirrors `invite_status_for()`'s "one function decides" convention.
    Called only from `PasswordChangeApiView`'s success path — any
    successful self-serve password change, temp or not, clears the flag
    that both `invite_status_for()` and the request-level gate
    (`config.views.BaseView`, issue #487) key off, since the whole point
    of the flag is "hasn't replaced the admin-generated password yet".
    """
