...
class PasswordResetRequestView(TemplateView):
    """`password-reset/`: a static, dead-end "contact an admin" page (issue #487, ADR 0018).

    The self-serve forgot-password flow this route used to serve is
    retired along with the emailed set-password link it depended on
    (`identity:set-password-confirm` no longer exists). The URL itself is
    kept live rather than deleted — per ADR 0018, a bookmark or a search-
    engine-cached link to it should land on an explanation, not a 404 —
    but it accepts no form input and sends no mail; the recovery path now
    is an admin using Reset password on the member's `/members/<pk>/` page,
    or `manage.py reset_password` for the sole-active-admin lockout case.
    """
