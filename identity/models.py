...
class Person(AbstractBaseUser, PermissionsMixin):
    ...
    not `has_usable_password()`. It's also the field the request-level
    gate (`config.views.BaseView`, issue #487) reads to refuse every
    authenticated route but `identity.api_views.PasswordChangeApiView`
