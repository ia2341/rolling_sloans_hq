...
| `must_change_password`, derived as `invite_status` (`'must_change_password'`/`'active'`, two-state since issue #482/ADR 0018) | ❌ never | ❌ never | ✅ read, in **read mode too** — a "must change password" badge on the row | Issue #455, narrowed by #482: every Membership now shows on this list regardless of credential state, so an admin needs a way to tell a not-yet-changed-password row apart from an active one; the derived status is never a raw flag, and is absent from the payload entirely for a Teammate or Self viewer, per the "absent, not null" wire contract |
...
**The Role-section carve-out is deliberately narrow (issue #378), widened by one more field each by issue #397's Invite action, issue #467's grant/revoke control, issue #468's future-scheduling-footprint read, and issue #469's Deactivate/Reactivate control.** Admin status unlocks write access to four things on this page — the Declared-roles editor, (since #397) the Invite action for a teammate who isn't `'accepted'` yet, (since #467) the admin grant/revoke control, and (since #469) the Deactivate/Reactivate control — plus one admin-only read, `future_scheduling_footprint` (issue #468) — and nothing else. Issue #483's Reset-password control adds no fifth payload field: it reuses the same "an admin viewing a teammate" condition `is_active`'s presence already signals, alongside Deactivate. Every other row in every table below keeps its ordinary Teammate/Self verdict for an admin viewer: an admin reading a teammate's page gets the same `email: never`, `Conflict: never`, `Recordings: never` (etc.) a plain teammate gets, with `can_edit_roles`/`available_roles`, `invite_status`, `is_admin`, `is_active`, and `future_scheduling_footprint` as the *only* extra capabilities/fields the payload grants. This mirrors #467's own `PersonAdminStatusApiView`: scoped, single-purpose admin actions bolted onto a read-only surface, never a page-wide "admin can edit everything" switch. `PersonApiViewerStateTests.test_admin_viewing_a_teammate_gets_the_teammate_key_set_except_can_edit_roles` (`scheduling/tests/test_person_page_visibility.py`) pins exactly this: an admin's payload differs from a teammate's by `can_edit_roles`/`available_roles`/`invite_status`/`is_admin`/`is_active`/`future_scheduling_footprint` alone, key for key.
...
| `password` | ❌ never | ❌ never | ❌ never | Self gets the `ChangePasswordRow`/`POST /api/password/` control instead; that control is self-only too |
| `last_login` | ❌ never | ❌ never | ❌ never | |
| `is_active` | ❌ never | ❌ never | ✅, Deactivate/Reactivate control only (issue #469) | Reverses this row's prior "never" verdict: `/accounts/manage/people/`'s retirement (issue #342, ADR 0017) moves the Deactivate/Reactivate lifecycle act here, so an admin viewing a teammate now reads the flag and can flip it via `PersonDeactivationApiView`/`PersonReactivationApiView`; still `never` for Self (self-deactivation is refused outright — `apply_person_deactivation()`'s self-guard) and `never` for a plain Teammate |
...
| `must_change_password`, derived as `invite_status` (`'must_change_password'`/`'active'`, two-state since issue #482/ADR 0018) | ❌ never | ❌ never | ✅, legacy "Invite"/"Invite again" action only | Issue #397, narrowed by #482: an admin viewing a teammate sees the derived status and the action it gates; never rendered as a raw flag, never shown to Self (a session implies `'active'`) or to a plain Teammate |
| Reset password control (`PersonPasswordResetApiView`, issue #483, ADR 0018) | ❌ never | ❌ never | ✅, same slot as Deactivate/admin grant-revoke | Same condition as those two controls (an admin viewing a teammate, never their own row): `apply_password_reset()` refuses a self-reset server-side (issue #483, narrowing ADR 0018's follow-on, which had allowed it) — an admin resets their own password through the self-serve `ChangePasswordRow` instead — so this control is never rendered for Self at all, matching the Deactivate/admin-status pattern rather than rendering-then-refusing. Never rendered for a plain Teammate either |
| The revealed temp password (`PersonPasswordResetApiView`'s `temp_password` response value) | ❌ never | ❌ never | ✅, once, in the post-action result dialog only | A new category this table hasn't needed before: a real, live, plaintext secret riding the wire transiently. It exists in the `POST /api/members/<pk>/reset-password/` write response and nowhere else — never re-fetchable, never persisted client- or server-side beyond that one response, never logged. The only other wire payload with this shape is Roster creation's `temp_passwords` (issue #482) |
...
| `future_scheduling_footprint` (other-Semester, not-yet-concluded `Membership`s and `SongRoleAssignment`s, plus future-dated `RehearsalSong`/`Backup`/Dress-Rehearsal Running Order appearances in any Semester) | ❌ never | ❌ never | ✅, Deactivate confirmation dialog only | Issue #468 (ADR-0017): the not-yet-built Deactivate act needs to show an admin what future commitments this Person still holds before cutting them short. A Semester is "concluded" (and excluded from the Membership/Assignment lists) only once every one of its Rehearsals is already past — `Semester` itself carries no date fields (ADR-0010), so this is derived from its Rehearsals, not from `published_at`. The Semester's Dress Rehearsal is included among the appearances (flagged `is_dress_rehearsal`) even though it carries no persisted `RehearsalSong` row (ADR-0003). Never shown to Self (a session already implies you know your own commitments, and self-deactivation is refused outright per ADR-0017) or to a plain Teammate. `is_role_mismatch` stays off this field entirely, same reasoning as the `SongRoleAssignment` table below |
