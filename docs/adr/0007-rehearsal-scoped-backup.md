...
## Amendment (issue #175): `attendance_suggestion_for()` and `breaks_for()` move with `attendance_for()`

This ADR named `attendance_for()` explicitly and ruled `performers_for()` out, but was silent on `services.attendance_suggestion_for()` and `services.breaks_for()`. Both are ruled **in**, on the same ground the original text gives for `attendance_for()`: a Backup "genuinely changes when that person must arrive and leave," which applies unchanged to a suggested arrival/departure window and to the idle gaps between a Person's slots. All three reads share one internal helper (`models.slots_for_person()`, issue #173), so this amendment is one change to that helper's definition rather than three separate edits that could disagree. `performers_for()` stays excluded, unchanged, per the Setlist-integrity line above.
