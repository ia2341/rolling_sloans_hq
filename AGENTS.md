...
- `identity/` owns the custom email-based `Person` user model: sign-in and account lifecycle, over just two server-rendered pages now (login, logout — issue #485 retired the rest). Accounts are never self-registered: an admin creates a `Person` (`create_person_with_temp_password()`) or resets an existing one (`apply_password_reset()`) with a real, immediately-usable temp password, revealed once in that synchronous response and relayed by the admin verbally or by text — never emailed, per ADR 0018. A `must_change_password` flag forces the member to pick their own password (via the SPA's change-password flow) before doing anything else. The project's first rate limit lives beside these in `identity/services.py`: failed sign-ins, keyed on the submitted address *and* the requesting IP, counted as `LoginAttempt` rows rather than cached (`CACHES` is unconfigured, so a cache would silently multiply any limit by the gunicorn worker count).
