...
- `identity/` owns the custom `Person` user model: sign-in over one server-rendered page, and two more that remain live but do nothing (`password-reset/` is a static "contact an admin" page; the old email-based invite/reset token route is gone entirely). Onboarding and recovery are admin-relayed temp passwords, not email (issue #487, ADR 0018): `create_person_with_temp_password()` (creation) and `apply_password_reset()` (an existing Person, `/members/<pk>/`'s Reset password action) each generate a real, immediately-usable password and flag `must_change_password`, revealing the plaintext once in the caller's own response for the admin to relay verbally or by text. `must_change_password` is enforced request-level by `config.views.BaseView`, which refuses every gated route but `PasswordChangeApiView` (the only route that clears it) to a Person still carrying one. `manage.py reset_password <email>` is the break-glass path for the sole-active-admin lockout case, deliberately outside the web app and carrying no admin-count guardrail. The project's one remaining rate limit lives beside these in `identity/services.py`: failed sign-ins, keyed on the submitted address *and* the requesting IP, counted as `LoginAttempt` rows rather than cached (`CACHES` is unconfigured, so a cache would silently multiply any limit by the gunicorn worker count).
