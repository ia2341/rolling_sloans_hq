"""The 0017 migration: a pre-existing zero-count SongRoleRequirement must not block the incoming check constraint."""
...
class DeleteZeroCountRequirementsMigrationTests(TransactionTestCase):
    """Migrates the real schema forward across 0017, the way it will run in production."""

    migrate_from = ('scheduling', '0016_backup')
    migrate_to = ('scheduling', '0017_alter_songrolerequirement_count_and_more')
