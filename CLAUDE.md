...
- **`services.deal_running_orders()`/`shuffle_rehearsal_running_order()` (issue #223) never re-deal a past Rehearsal or a Recording-bearing row.** A past Rehearsal is excluded outright (generation cannot rewrite history, mirroring the date generator). A row with a Recording is pinned at its exact identity, `order` and `slot_count` — the dealer fills every other slot around it rather than through it, so a bulk run can never invalidate an upload's timing. Both functions are pure reads (no `apply_schedule_generation`, deliberately: see `RehearsalDeal`'s docstring) that fill the Rehearsal editor's existing Pending Buffer the same way a hand-edit does; only `apply_rehearsal_edits()` ever writes a `RehearsalSong` row.
